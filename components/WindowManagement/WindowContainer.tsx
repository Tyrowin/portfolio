import type { RefObject, MutableRefObject, FormEvent } from 'react';
...
const Resizable = (props: {
  ...
  isMaximized: MutableRefObject<boolean>;
}) => {
    ...
    resizeAxis.split('').forEach(x => {
      switch (x) {
        case 'n':
          ...
          break;
        case 's':
          origin.current.offset.y =
            origin.current.cursor.y - (correctedWindowY + windowData.height);
          break;
        case 'w':
          origin.current.offset.x = origin.current.cursor.x - correctedWindowX;
          break;
        case 'e':
          origin.current.offset.x =
            origin.current.cursor.x - (correctedWindowX + windowData.width);
          break;
      }
    });
  ...
  function onPointerMove(evt: PointerEvent) {
    ...
    const [deltaX, deltaY] = [
      origin.current.cursor.x -
        clamp(
          evt.clientX,
          parent.offsetLeft + offsetX,
          parent.offsetWidth + offsetX
        ),
      origin.current.cursor.y -
        clamp(
          evt.clientY,
          parent.offsetTop + offsetY,
          parent.offsetTop + parent.offsetHeight + offsetY
        ),
    ];
    ...
    const resized =
      original.height !== windowData.height ||
      original.width !== windowData.width;
  ...
  return (
    <>
      ...
      {resizing && (
        <div
          data-interactive-window
          className={styles['resizing-mask']}
          style={resizableStyle}
        ></div>
      )}
    </>
...
const MinimizeIcon =
  'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAA0AAAANCAYAAABy6+R8AAAAAXNSR0IArs4c6QAAAC5JREFUKJFjYBh+gBFK/ydFDyMShxiNjMg2EaOREYNBQCMjTg4OjbjUENQ4lAAAsBUGBRYmg2IAAAAASUVORK5CYII=';
const MaximizeIcon =
  'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAA0AAAANCAYAAABy6+R8AAAAAXNSR0IArs4c6QAAADBJREFUKJFjYBhZ4D8uCUYiNGCowaYJmw2MODn4nISsFlkTPg0o6mGaiNGAzaLBCACuAwYFQV/6vgAAAABJRU5ErkJggg==';
const CloseIcon =
  'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAA0AAAANCAYAAABy6+R8AAAAAXNSR0IArs4c6QAAAEhJREFUKJFjYKAA/IdiotQwYZHApQEOGHFI4BVHlsSmAKtB6JpwOZERJwefkwhpImgTvtDDFRiUhR5eP6DLozsPV8DgEiceAADT2BMJIsMZOQAAAABJRU5ErkJggg==';
...
const WindowHeader = (
  ...
  const originalWindow = useRef<OriginWindow>({
    x: 0,
    y: 0,
    width: 0,
    height: 0,
  ...
  function onPointerDown(evt: PointerEvent) {
    ...
    const windowRoot = output.current!.parentNode!.parentNode!
      ...
      <div ref={output} data-interactive-window className={classes.join(' ')}>
        ...
        <div className={styles['header-buttons']}>
          <button
            ...
            onClick={() => {
              windowCompositor.minimize(windowData.id);
            }}
          >
            <img src={MinimizeIcon} alt="Minimize window" draggable={false} />
          </button>
          <button className="header-button" onClick={onClickMaximize}>
            <img src={MaximizeIcon} alt="Maximize window" draggable={false} />
          </button>
          <button
            className="header-button"
            onClick={() => {
              windowCompositor.close(windowData.id);
            }}
          >
            <img src={CloseIcon} alt="Close window" draggable={false} />
          </button>
        </div>
      </div>
...
function HandleWindowActionPrompt(props: {
  prompt: WindowActionPrompt | null;
}) {
...
function HandleWindowAction(props: { action: WindowAction | null }) {
  ...
  return (
    <>
      <HandleWindowActionPrompt
        prompt={action?.action === 'prompt' ? action : null}
      />
      <HandleWindowActionAlert
        alert={action?.action === 'alert' ? action : null}
      />
    </>
  );
}
  ...
  return (
    <div style={style} data-window-root="true">
      <div className={styles.container}>
        {!window.focused && (
          <div onPointerDown={focus} className={styles['focus-layer']}></div>
        )}
        {window.focused && (
          <Resizable
            windowData={window}
