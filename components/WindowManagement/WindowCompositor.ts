...
import {
  createAllWindowsClosedEvent,
  createWindowCloseEvent,
  createWindowOpenEvent,
} from '@/applications/ApplicationEvents';
...
export interface WindowActionPrompt {
  action: 'prompt';
  prompt: string;
  defaultValue?: string;
...
export interface WindowActionAlert {
  action: 'alert';
  alert: string;
  resolve: () => void;
  reject: (reason: string) => void;
...
export class Window {
  ...
  public minimalWidth = 240;
  public minimalHeight = 180;
...
export class WindowCompositor {
  private windowId = 0;
  private windows = new Chain<Window>();
  ...
  public open(config: WindowConfig): Window {
    function getWindowsOfTheSameApplication(
      chain: Chain<Window>,
      config: WindowConfig
    ) {
    ...
    do {
      ...
      const collidedWindow = windows.find(
        entry => entry.x === x && entry.y === y
      );

      if (collidedWindow) {
        const widthFitsInView =
          x + width + WindowCollisionMoveDistance <= this.viewWidth;
        const heightFitsInView =
          y + height + WindowCollisionMoveDistance <= this.viewHeight;
  ...
  public update(
    window: Window,
    updateOptions?: { moved?: boolean; resized?: boolean }
  ) {
  ...
  public minimize(windowId: number): void {
    ...
    const instances = this.listPreviousNodesOfSameApplication(
      window.application
    );
    const nonMinimizedWindow = instances.find(x => !x.value.minimized);
  ...
  private listPreviousNodesOfSameApplication(
    application: Application
  ): Node<Window>[] {
    let node = this.windows.getHead();
    const results: Node<Window>[] = [];
  ...
  private findPreviousNodeOfSameApplication(
    application: Application
  ): Node<Window> | null {
    let node = this.windows.getHead();
  ...
  private updateWindowOrder(): void {
    function focusLastVisibleNode(
      lastNode: Node<Window>,
      applicationManager: ApplicationManager
    ): void {
      let node: Node<Window> | null = lastNode;

      while (node) {
        const isMinimized = node.value.minimized;
    ...
    if (node === null) {
      ...
      const finder = applications.find(
        x => x.config().appName === 'Finder.app'
      );

      if (finder) {
        this.applicationManager.focus(finder);
      }
