...
function handleParentResponsesClosure(
  ...
  return function (event: MessageEvent) {
    ...
    return; // Skip RPC handling

    // RPC message handling commented out - no longer needed
    // switch (value.method) {
...
export const OperatingSystem = () => {
  ...
  function handleTouchStartEvent(evt: TouchEvent) {
    ...
    // sendRequestToParent({ method: 'camera_zoom_distance_request' }); // RPC removed
  ...
  function handleMoveCameraEvent(
    camera: Camera,
    coords: PointerCoordinates
  ): [number, number] {
    if (touchOrigin.current === null) {
      return [0, 0];
    }
    ...
    return [
      ...
      camera.verticalOffset + vertical,
    ];
  }

  function handleZoomCameraEvent(
    camera: Camera,
    origin: TouchData,
    data: TouchData
  ): number {
  ...
  function handleTouchMoveEvent(evt: TouchEvent) {
    ...
    if (camera.current === null || initialCamera.current === null) {
      return;
    }
    ...
    if (isTouchMoveCamera(data)) {
      [cam.horizontalOffset, cam.verticalOffset] = handleMoveCameraEvent(
        initialCam,
    ...
    // Send *potential* camera state to parent - RPC removed
    // sendRequestToParent({
  ...
  useEffect(() => {
    ...
    if (system.isDebug()) {
      addDebugAppToFileSystem(fileSystem);
    }
    ...
    return () => {
      ...
      if (system.isDebug()) {
        removeDebugAppFromFileSystem(fileSystem);
      }
