...
export function SortingGenerationStrategyDropdown(
  ...
  const entries: DataGenerationEntry<SortingDataGenerationStrategy>[] = [
    {
      title: t('data_generation_options.randomly_distributed'),
      strategy: 'randomly-distributed',
    },
    {
      title: t('data_generation_options.sorted_left_to_right'),
      strategy: 'sorted-left-to-right',
    },
    {
      title: t('data_generation_options.sorted_right_to_left'),
...
export function PathFindingGenerationStrategyDropdown(
  id: string,
  value: string,
  ...
  const entries: DataGenerationEntry<PathFindingDataGenerationStrategy>[] = [
    {
      title: t('path_finding_generation_options.maze'),
      strategy: 'maze',
    },
    {
      title: t('path_finding_generation_options.open_field'),
      strategy: 'open-field',
    },
    {
      title: t('path_finding_generation_options.pipes'),
      strategy: 'pipes',
    },
  ];
...
export function DataGenerationStrategyDropDown<T>(
  id: string,
  entries: DataGenerationEntry<T>[],
  value: string,
  onChange: (strategy: T) => void
) {
...
export function DataGenerationEntriesInput(
  id: string,
  value: number | null,
  onChange: (entries: number | null) => void,
  min: number,
  max: number
) {
...
export default function HomeSubView(params: SubViewParams) {
  const [sortingGenerationStrategy, setSortingGenerationStrategy] =
    useState<SortingDataGenerationStrategy>('randomly-distributed');
  const [sortingGenerationEntries, setSortingGenerationEntries] = useState<
    number | null
  >(50);

  const [pathFindingGenerationStrategy, setPathFindingGenerationStrategy] =
    useState<PathFindingDataGenerationStrategy>('maze');
    ...
    <>
      ...
      >
        <div data-subpage-content>
          ...
          <h3>{t('sorting_options')}</h3>

          <table>
            <tbody>
              <tr>
                <td>
                  <label htmlFor="data-generation-strategy">
                    {t('data_generation_strategy')}
                  </label>
                </td>
                <td>
                  {SortingGenerationStrategyDropdown(
                    'data-generation-strategy',
                    sortingGenerationStrategy,
                    setSortingGenerationStrategy,
                    t
                  )}
                </td>
              </tr>
              ...
              <tr>
                <td>
                  <label htmlFor="generated-data-size">
                    {t('data_generation_points')}
                  </label>
                </td>
                <td>
                  {DataGenerationEntriesInput(
                    'generated-data-size',
                    sortingGenerationEntries,
                    setSortingGenerationEntries,
                    1,
                    2048
                  )}
                </td>
          ...
          <table>
            <tbody>
              <tr>
                <td>
                  <label htmlFor="path-finding-data-gen-strategy">
                    {t('data_generation_strategy')}
                  </label>
                </td>
                <td>
                  {PathFindingGenerationStrategyDropdown(
                    'path-finding-data-gen-strategy',
                    pathFindingGenerationStrategy,
                    setPathFindingGenerationStrategy,
                    t
                  )}
                </td>
              </tr>
              ...
              <tr>
                <td>
                  <label htmlFor="path-finding-width">
                    {t('path_finding_width')}
                  </label>
                </td>
                <td>
                  {DataGenerationEntriesInput(
              ...
              <tr>
                <td>
                  <label htmlFor="path-finding-height">
                    {t('path_finding_height')}
                  </label>
                </td>
                <td>
                  {DataGenerationEntriesInput(
