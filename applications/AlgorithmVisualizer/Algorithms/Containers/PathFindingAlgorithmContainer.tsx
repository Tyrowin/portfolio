...
function generateData(
  strategy: PathFindingDataGenerationStrategy,
  width: number,
  height: number
): Area {
  switch (strategy) {
    ...
    case 'pipes':
      return generatePipes(width, height);
  }
}

export function PathFindingAlgorithmContainer(
  props: PathFindingAlgorithmContainerProps
) {
  ...
  const [pathFindingGenerationStrategy, setPathFindingGenerationStrategy] =
    useState(props.options.pathFinding.dataGenerationStrategy);
  const [pathFindingWidth, setPathFindingWidth] = useState<number | null>(
    props.options.pathFinding.width
  );
  const [pathFindingHeight, setPathFindingHeight] = useState<number | null>(
    props.options.pathFinding.height
  );
  ...
  function onPointerEvt(evt: PointerEvent) {
    ...
    area.setTile(
      tileX,
      tileY,
      area.getTile(tileX, tileY) === 'wall' ? 'open' : 'wall'
    );
  ...
  const actionButton = isPathFinding ? (
    <button className="xl-system-button" onClick={onStop}>
      {t('stop')}
    </button>
  ) : (
    <button className="xl-system-button" onClick={onStart}>
      {t('start')}
    </button>
  );
