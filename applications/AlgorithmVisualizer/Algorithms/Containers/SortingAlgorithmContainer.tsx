...
function generateData(
  strategy: SortingDataGenerationStrategy,
  entries: number
): SortViewEntry[] {
  switch (strategy) {
    case 'randomly-distributed':
      return generateRandomData(entries);
    case 'sorted-left-to-right':
      return generateSortedDataLeftToRight(entries);
    case 'sorted-right-to-left':
      return generateSortedDataRightToLeft(entries);
  }
}

export function SortingAlgorithmContainer(
  props: SortingAlgorithmContainerProps
) {
  ...
  const [dataGenStrategy, setDataGenStrategy] = useState(
    props.options.sorting.dataGenerationStrategy
  );
  const [amountOfEntries, setAmountOfEntries] = useState<number | null>(
    props.options.sorting.amountOfEntries
  );
