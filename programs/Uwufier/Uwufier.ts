...
const NYAIFY_THRESHOLD = 0.9;
const STUTTER_THRESHOLD = 0.25;
const EMOJIFY_THRESHOLD = 0.85;

const replacementsMap: [string, string][] = [
  ['small', 'smol'],
  ['cute', 'kawaii~'],
  ['fluff', 'floof'],
  ['stupid', 'baka'],
...
function replaceWords(words: string[], changeTracker: Set<number>): string[] {
  ...
  return words.map((word, index) => {
    ...
    for (const [pattern, replacement] of replacementsMap) {
      const searchIndex = searchWord.indexOf(pattern);
      if (searchIndex < 0) {
        continue;
      }
...
function stutter(words: string[], rng: PRNG): string[] {
  ...
  return words.map(word => {
    if (!passesThreshold(rng, STUTTER_THRESHOLD)) {
      return word;
    }
...
function emojify(words: string[], rng: PRNG): string[] {
  ...
  return words.map((word, index) => {
    if (!passesThreshold(rng, EMOJIFY_THRESHOLD)) {
      return word;
    }
    if (!canEmojify(words, index)) {
      return word;
    }
